      ...
      <div className="mx-auto max-w-3xl rounded-md border border-slate-200 bg-white p-10 print:border-0 print:p-0 print:shadow-none dark:border-slate-700 dark:bg-slate-900">
        <div className="flex items-start justify-between border-b border-slate-200 pb-6 dark:border-slate-700">
          ...
          <div className="text-right text-sm text-slate-600 dark:text-slate-400">
            <p className="font-medium text-slate-900 dark:text-white">{selectedYear.name}</p>
        ...
        <div className="mt-6 grid grid-cols-3 gap-4">
          ...
          <div className="rounded-lg border border-slate-200 p-4 dark:border-slate-700">
            ...
            <p className="mt-1 text-2xl font-semibold text-slate-900 dark:text-white">{targets.filter((t) => t.status === "ACHIEVED").length}/{targets.length}</p>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {assessmentResults.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="py-2 text-slate-600 dark:text-slate-400">{r.notes ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="mt-10 border-t border-slate-200 pt-6 dark:border-slate-700">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Teacher comment</h2>
          <TeacherComment />
        </section>
      </div>
    </div>
