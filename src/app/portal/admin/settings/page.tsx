          ...
          <div className="mt-4 space-y-4">
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-4">
                <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-md border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
                  {tenant.logoUrl ? (
                ...
                <label className="cursor-pointer rounded-md border border-slate-300 px-3 py-1.5 text-sm text-slate-600 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-400 dark:hover:bg-slate-800">
                ...
                {tenant.logoUrl && (
                  <button onClick={removeLogo} disabled={uploadingLogo} className="text-sm text-red-600 hover:underline disabled:opacity-50 dark:text-red-400">
                    Remove
                  </button>
                )}
              </div>
              <p className="mt-1 text-xs text-slate-700 dark:text-slate-200">Shown in the portal navigation bar. PNG, JPEG, SVG, WebP, or GIF, up to 2MB.</p>
            </div>
            <div>
              ...
              <input
                ...
                className="mt-1 h-10 w-16 rounded-md border border-slate-300 dark:border-slate-600"
              />
            </div>
            <div className="border-t border-slate-100 pt-4 dark:border-slate-800">
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">App name (optional)</label>
              <input
            ...
            <div>
              ...
              {tenant.sidebarColor && (
                <input
                  ...
                  className="mt-2 h-10 w-16 rounded-md border border-slate-300 dark:border-slate-600"
                />
              )}
            </div>
            <div className="border-t border-slate-100 pt-4 dark:border-slate-800">
              <p className="text-sm font-medium text-slate-900 dark:text-white">Nav modules</p>
              <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">Hide modules this school doesn&apos;t use from the staff navigation.</p>
              <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">
                {TOGGLEABLE_NAV_ITEMS.map((item) => (
                  <label key={item.href} className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                    <input
                      type="checkbox"
