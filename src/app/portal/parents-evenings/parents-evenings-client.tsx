...
export function ParentsEveningsClient({ formGroups, teachers }: { formGroups: FormGroup[]; teachers: Teacher[] }) {
  ...
  return (
    <div>
      ...
      {showForm && (
        <form onSubmit={submitEvent} className="mt-4 grid grid-cols-1 gap-3 rounded-md border border-slate-200 bg-white p-5 sm:grid-cols-2 dark:border-slate-700 dark:bg-slate-900">
          ...
          <input value={locationNote} onChange={(e) => setLocationNote(e.target.value)} placeholder="Location note (optional)" className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2 dark:border-slate-600" />

          <div className="sm:col-span-2">
            <p className="mb-2 text-xs font-medium uppercase tracking-wide text-slate-600 dark:text-slate-400">Form groups</p>
            <div className="flex flex-wrap gap-3">
              {formGroups.map((fg) => (
                <label key={fg.id} className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                  <input type="checkbox" checked={selectedFormGroups.includes(fg.id)} onChange={() => toggleFormGroup(fg.id)} />
                  {fg.name}
                </label>
              ))}
              {formGroups.length === 0 && <p className="text-sm text-slate-600 dark:text-slate-400">No form groups set up yet.</p>}
            </div>
          </div>
      ...
      <div className="mt-4 overflow-hidden rounded-md border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            ...
            {events?.map((ev) => (
              <tr key={ev.id}>
                <td className="p-4 font-medium text-slate-900 dark:text-white">{ev.title}</td>
                <td className="p-4 text-slate-600 dark:text-slate-400">{new Date(ev.date).toLocaleDateString("en-GB")}</td>
  ...
  return (
    <div className="mt-6 rounded-md border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-slate-900 dark:text-white">{event?.title ?? "Loading…"}</h2>
        <button onClick={onClose} className="text-sm text-slate-600 hover:underline dark:text-slate-400">Close</button>
      </div>

      <form onSubmit={generateSlots} className="mt-4 grid grid-cols-1 gap-3 rounded-lg bg-slate-50 p-4 sm:grid-cols-4 dark:bg-slate-950">
        <select value={teacherId} onChange={(e) => setTeacherId(e.target.value)} className="rounded-md border border-slate-300 px-3 py-2 text-sm dark:border-slate-600">
          {teachers.map((t) => (
            <option key={t.id} value={t.id}>{t.name ?? t.email}</option>
          ))}
        </select>
        <input type="time" value={rangeStart} onChange={(e) => setRangeStart(e.target.value)} className="rounded-md border border-slate-300 px-3 py-2 text-sm dark:border-slate-600" />
        <input type="time" value={rangeEnd} onChange={(e) => setRangeEnd(e.target.value)} className="rounded-md border border-slate-300 px-3 py-2 text-sm dark:border-slate-600" />
      ...
      <div className="mt-4 overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            ...
            {slots?.map((s) => (
              <tr key={s.id}>
                <td className="p-3 text-slate-700 dark:text-slate-200">{s.teacher.name ?? s.teacher.email}</td>
                <td className="p-3 text-slate-600 dark:text-slate-400">{fmtTime(s.startTime)}–{fmtTime(s.endTime)}</td>
                <td className="p-3">
